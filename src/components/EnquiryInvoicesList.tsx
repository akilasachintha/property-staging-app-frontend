...
const EnquiryInvoiceList: FC<EnquiryListProps> = ({rowsCount}) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage] = useState(rowsCount);
    const [searchTerm, setSearchTerm] = useState('');
    const navigate = useNavigate();
    const {enquiryInvoiceList, getInvoicesByEnquiryId} = useEnquiryContext();
    ...
    const enquiries =
        enquiryInvoiceList &&
        enquiryInvoiceList.length > 0 &&
        enquiryInvoiceList.map((invoice: InvoiceType) => ({
            id: invoice.id,
    ...
    useEffect(() => {
        if (id) {
            getInvoicesByEnquiryId && getInvoicesByEnquiryId(id).catch((error) => console.log(error.message));
        }
    }, [id]);

    if (!enquiryInvoiceList) {
...
export default EnquiryInvoiceList;
